...
class TeamSpeak3
{
	...
	public static function factory($uri)
	{
		...
		$uri = new \TeamSpeak3\Helper\Uri($uri);
		...
		$adapter_clean = '\\' . str_replace('_', '\\', $adapter);

		/* @var $adapter object */
		$object = new $adapter_clean($options);
		
		if ($object instanceof Adapter\ServerQuery)
		{
			$node = $object->getHost();
			...
			if ($node instanceof \TeamSpeak3\Node\Server)
			{
	...
	public static function init()
	{
		...
		if (!class_exists("\TeamSpeak3\Helper\Profiler"))
		{
			spl_autoload_register(array(__CLASS__, "autoload"));
		}

		\TeamSpeak3\Helper\Profiler::start();
	}

...
class Html implements viewerInterface
{
  ...
  protected function getCorpusTitle()
  {
    ...
    {
      return "ID: " . $this->currObj->getId() . " | Clients: " . $this->currObj->clientCount() . "/" . $this->currObj["virtualserver_maxclients"] . " | Uptime: " . \TeamSpeak3\Helper\Convert::seconds($this->currObj["virtualserver_uptime"]);
    }
    elseif($this->currObj instanceof \TeamSpeak3\Node\Channel && !$this->currObj->isSpacer())
    {
      return "ID: " . $this->currObj->getId() . " | Codec: " . \TeamSpeak3\Helper\Convert::codec($this->currObj["channel_codec"]) . " | Quality: " . $this->currObj["channel_codec_quality"];
    }
    elseif($this->currObj instanceof \TeamSpeak3\Node\Client)
    {
      return "ID: " . $this->currObj->getId() . " | Version: " . \TeamSpeak3\Helper\Convert::versionShort($this->currObj["client_version"]) . " | Platform: " . $this->currObj["client_platform"];
    }
    elseif($this->currObj instanceof \TeamSpeak3\Node\ervergroup || $this->currObj instanceof \TeamSpeak3\Node\Channelgroup)
    {
      return "ID: " . $this->currObj->getId() . " | Type: " . \TeamSpeak3\Helper\Convert::groupType($this->currObj["type"]) . " (" . ($this->currObj["savedb"] ? "Permanent" : "Temporary") . ")";
    }
  }
  ...
  {
    if($this->currObj instanceof \TeamSpeak3\Node\Channel && $this->currObj->isSpacer()) return;
  ...
  protected function getSuffixIconServer()
  {
    ...
    if($this->currObj["virtualserver_icon_id"])
    {
      if(!$this->currObj->iconIsLocal("virtualserver_icon_id") && $this->ftclient)
      {
        ...
        if($this->ftclient == "data:image")
        {
          $html .= $this->getImage("data:" . \TeamSpeak3\Helper\Convert::imageMimeType($download) . ";base64," . base64_encode($download), "Server Icon", null, FALSE);
        }
        else
        {
  ...
  protected function getSuffixIconChannel()
  {
    ...
    if($this->currObj["channel_icon_id"])
    {
      if(!$this->currObj->iconIsLocal("channel_icon_id") && $this->ftclient)
      {
        ...
        if($this->ftclient == "data:image")
        {
          $html .= $this->getImage("data:" . \TeamSpeak3\Helper\Convert::imageMimeType($download) . ";base64," . base64_encode($download), "Channel Icon", null, FALSE);
        }
        else
        {

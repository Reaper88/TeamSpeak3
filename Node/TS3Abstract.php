 ...
 */
abstract class TS3Abstract implements \RecursiveIterator, \ArrayAccess, \Countable
{
  /**
   * @ignore
   */
  ...
  public function iconGetName($key)
  {
    ...
    return new \TeamSpeak3\Helper\String("/icon_" . $iconid);
  }
  ...
  {
    if($this instanceof \TeamSpeak3\Node\Channel && $this->isSpacer())
    {
      return $prefix . "spacer";
    }
    elseif($this instanceof \TeamSpeak3\Node\Client && $this["client_type"])
    {
      return $prefix . "query";
    }
	$class = $prefix . \TeamSpeak3\Helper\String::factory(get_class($this))->toLower()->section("_", 2);
   ...
   * @return string
   */
  public function getViewer(\TeamSpeak3\Viewer\viewerInterface $viewer)
  {
  ...
  protected function filterList(array $nodes = array(), array $rules = array())
  {
    if(!empty($rules))
    {
      ...
      {
        if(!$node instanceof TS3Abstract) continue;
        ...
        {
          if($val instanceof \TeamSpeak3\Helper\String)
          {
  ...
  public function getInfo($extend = TRUE, $convert = FALSE)
  {
    ...
    if($convert)
    {
      ...
      {
        $key = \TeamSpeak3\Helper\String::factory($key);

        if($key->contains("_bytes_"))
        {
          $info[$key->toString()] = TeamSpeak3\Helper\Convert::bytes($val);
        }
        elseif($key->contains("_bandwidth_"))
        {
          $info[$key->toString()] = \TeamSpeak3\Helper\Convert::bytes($val) . "/s";
        }
        ...
        {
          $info[$key->toString()] = \TeamSpeak3\Helper\Convert::seconds($val);
        }
        elseif($key->endsWith("_version"))
        {
          $info[$key->toString()] = \TeamSpeak3\Helper\Convert::version($val);
        }
  ...
  {
    if($this->getParent() instanceof TS3Abstract)
    {

 ...
 */
class Server extends TS3Abstract
{
  /**
   * @ignore
   */
   ...
   */
  public function __construct(\TeamSpeak3\Node\Host $host, array $info, $index = "virtualserver_id")
  {
    $this->parent   = $host;
    $this->nodeInfo = $info;
  ...
  public function channelDelete($cid, $force = FALSE)
  {
    ...
    if(($cid instanceof TS3Abstract ? $cid->getId() : $cid) == $this->whoamiGet("client_channel_id"))
    {
  ...
  public function channelSpacerCreate($ident, $type = TeamSpeak3::SPACER_SOLIDLINE, $align = TeamSpeak3::SPACER_ALIGN_REPEAT, $order = null, $maxclients = 0)
  {
    ...
    switch($align)
    {
      ...
      default:
        throw new \TeamSpeak3\Adapter\ServerQuery\Exception("missing required parameter", 0x606);
        break;
    }
  ...
  public function channelSpacerGetType($cid)
  {
    ...
    {
      throw new \TeamSpeak3\Adapter\ServerQuery\Exception("invalid channel flags", 0x307);
    }
  ...
  public function channelFileList($cid, $cpw = "", $path = "/", $recursive = FALSE)
  {
    ...
    for($i = 0; $i < $count; $i++)
    {
      ...
      $files[$i]["src"]  = new \TeamSpeak3\Helper\String($cid ? $files[$i]["path"] : "/");
  ...
  public function channelGetById($cid)
  {
    ...
    {
      throw new \TeamSpeak3\Adapter\ServerQuery\Exception("invalid channelID", 0x300);
    }
  ...
  public function clientGetById($clid)
  {
    ...
    {
      throw new \TeamSpeak3\Adapter\ServerQuery\Exception("invalid clientID", 0x200);
    }
  ...
  public function clientMove($clid, $cid, $cpw = null)
  {
    ...
    if($clid instanceof TS3Abstract)
    {
      $clid = $clid->getId();
    }

    if($cid instanceof TS3Abstract)
    {
      $cid = $cid->getId();
    }
  ...
  public function transferInitDownload($clientftfid, $cid, $name, $cpw = "", $seekpos = 0)
  {
    ...
    {
      throw new \TeamSpeak3\Adapter\ServerQuery\Exception($download["msg"], $download["status"]);
    }
    ...
    \TeamSpeak3\Helper\Signal::getInstance()->emit("filetransferDownloadInit", $download["ftkey"], $download);
   ...
   * @return void
   */
  public function clientMessage($id, $msg)
  {
    $this->execute("sendtextmessage", array("msg" => $msg, "target" => $id, "targetmode" => TeamSpeak3::TEXTMSG_CLIENT));
  }
  ...
  protected static function sortClientList(Client $a, Client $b)
  {
    ...
    if(!$a instanceof TeamSpeak3_Node_Client)
    {
      ...
      /* workaround for PHP bug #50688 */
      throw new \TeamSpeak3\Adapter\ServerQuery\Exception("convert error", 0x604);
    }
   ...
   * @return integer
   */
  protected static function sortGroupList(TS3Abstract $a, TS3Abstract $b)
  {
    if(get_class($a) != get_class($b))
    {
      return 0;

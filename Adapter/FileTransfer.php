 ...
 */
class FileTransfer extends TS3Abstract
{
  /**
  ...
  public function syn()
  {
    ...
    \TeamSpeak3\Helper\Profiler::init(spl_object_hash($this));

    \TeamSpeak3\Helper\Signal::getInstance()->emit("filetransferConnected", $this);
  }
  ...
  {
    if($this->getTransport() instanceof \TeamSpeak3\Transport\TS3Abstract && $this->getTransport()->isConnected())
    {
  ...
  protected function init($ftkey)
  {
    ...
    \TeamSpeak3\Helper\Signal::getInstance()->emit("filetransferHandshake", $this);
  }
  ...
  public function upload($ftkey, $seek, $data)
  {
    ...
    \TeamSpeak3\Helper\Signal::getInstance()->emit("filetransferUploadStarted", $ftkey, $seek, $size);
    ...
    for(;$seek < $size;)
    {
      ...
      \TeamSpeak3\Helper\Signal::getInstance()->emit("filetransferUploadProgress", $ftkey, $seek, $size);
    }

    $this->getProfiler()->stop();

    \TeamSpeak3\Helper\Signal::getInstance()->emit("filetransferUploadFinished", $ftkey, $seek, $size);
  ...
  protected function passthru($size)
  {
    ...
    if($buff_size != $size)
    {
      throw new TeamSpeak3\Adapter\FileTransfer\Exception("incomplete file download (" . intval($buff_size) . " of " . $size . " bytes)");
    }
  }
}

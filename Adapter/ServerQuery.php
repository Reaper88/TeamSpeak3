 ...
 */
class ServerQuery extends TS3Abstract
{
  /**
  ...
  protected function syn()
  {
    ...
    \TeamSpeak3\Helper\Signal::getInstance()->emit("serverqueryConnected", $this);
  }
  ...
  {
    if($this->getTransport() instanceof \TeamSpeak3\Transport\TS3Abstract && $this->transport->isConnected())
    {
      try
      {
        $this->request("quit");
    ...
    foreach($params as $ident => $value)
    {
      ...
      if(is_array($value))
      {
        ...
        for($i = 0; $i < count($value); $i++)
        {
          ...
          elseif($value[$i] instanceof \TeamSpeak3\Node\TS3Abstract) $value[$i] = $value[$i]->getId();
  ...
  public function getHost()
  {
    if($this->host === null)
    {
      $this->host = new \TeamSpeak3\Node\Host($this);
    }
